...
<html>
<head>
   <script type="text/javascript" src="https://maps.googleapis.com/maps/api/js?key=AIzaSyDYvT3Iy5KLWcKNnkHL8hgAfZGWvLq4OSw&libraries=places"></script>
...
if(mysqli_num_rows($query) > 0) {
    while($rows = mysqli_fetch_assoc($query)) {         
        ...
        $district = isset($rows['district']) ? $rows['district'] : '';
          ...
          <div class="row">
            <div class="col-sm-6">
              <table>
                ...
                <tr>
                  <td><h3>Thana:</h3></td>
                  <td><h4><?php echo $rows['thana']; ?></h4></td>
                </tr>
                <tr>
                ...
                <tr>
                  ...
                  <td><h4>Rs.<?php echo $rows['asking_price']; ?></h4></td>
                </tr>
              </table>
            </div>
          </div>
...
if(isset($_SESSION["email"]) && !empty($_SESSION['email'])) {
    ...
    $check_tenant = "SELECT * FROM tenant WHERE email = '$user_email'";
    $tenant_result = mysqli_query($db, $check_tenant);
...
?>
            <div class="row">
              <div class="col-sm-6">
                <button class="btn btn-lg btn-primary" style="width: 100%; margin-bottom: 20px;" onclick="showPaymentForm()">Book Now</button>
                <div id="payment-section" style="display: none;">
                  <form id="payment-form">
                    <input type="hidden" name="property_id" value="<?php echo $rows['property_id']; ?>">
                    <input type="hidden" name="amount" value="<?php echo $rows['asking_price']; ?>">
                    <div id="payment-element"></div>
                    <button id="submit-payment">Pay Rs.<?php echo $rows['asking_price']; ?></button>
...
<?php 
        ...
        echo "<div class='alert alert-warning' role='alert'>Only tenants can book properties. Please login as a tenant to proceed.</div>";
    }
} else {
    echo "<div class='alert alert-info' role='alert'>Please <a href='tenant-login.php' class='alert-link'>login as a tenant</a> to book this property.</div>";
}
?>
...
<?php 
    ...
    echo "<center><h3>Property not found.</h3></center>";
}
?>

<br>
<!-- Move lat/lon outside the map div and use hidden elements -->
<input type="hidden" id="lat" value="<?php echo $rows['latitude'];?>">
<input type="hidden" id="lon" value="<?php echo $rows['longitude'];?>">
...
<?php if(isset($_SESSION["email"]) && !empty($_SESSION['email'])) {
    ...
    $is_owner = mysqli_num_rows($owner_result) > 0;
} ?>
<?php if($is_owner) { ?>
    <div class="alert alert-info" style="margin-top: 10px;">
        <strong>Property Owner:</strong> Click on the map to update the property location.
    </div>
    <form id="updateLocationForm" method="POST" action="update-location.php" style="display: none;">
        <input type="hidden" name="property_id" value="<?php echo $property_id; ?>">
        <input type="hidden" name="latitude" id="update_lat">
        <input type="hidden" name="longitude" id="update_lon">
...
function initialize() {
    ...
    if (isNaN(x) || isNaN(y)) {
        // Fallback: center map at a default location if coordinates are invalid
        x = 23.8103; // Example: Dhaka
        y = 90.4125;
    }
    ...
    <?php if($is_owner) { ?>
    // Add click listener to map for property owners
    google.maps.event.addListener(map, 'click', function(event) {
        // Update marker position
        marker.setPosition(event.latLng);
        
        // Update hidden form fields
        document.getElementById('update_lat').value = event.latLng.lat();
        document.getElementById('update_lon').value = event.latLng.lng();
        ...
        // Update info window
        var iwContent = '<div id="iw_container">' +
            '<div class="iw_title"><b>New Location</b></div>' +
            '<div>Click "Save New Location" to update</div>' +
            '</div>';
        infowindow.setContent(iwContent);
        infowindow.open(map, marker);
    });
...
async function initialize() {
  const { clientSecret } = await fetch("process-payment.php", {
    method: "POST",
...
async function handleSubmit(e) {
  ...
  const { error } = await stripe.confirmPayment({
    elements,
    confirmParams: {
      return_url: `${window.location.origin}/House-Rental-System-main/renthouse/payment-success.php`,
    },
  });
...
<script>
// Add this new function to show/hide payment form
function showPaymentForm() {
  document.getElementById('payment-section').style.display = 'block';
  document.querySelector('.btn-primary').style.display = 'none';
  initialize(); // Initialize Stripe elements when showing the form
}
</script>